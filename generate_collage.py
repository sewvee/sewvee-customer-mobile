    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen">
      ...
      <View style={s.root}>
        ...
        <View style={s.canvasArea}>
              ...
              {(selectedLayout.id === '3_grid' || selectedLayout.id === '4_grid') ? (
                <>
                  <View style={{ flex: 1, flexDirection: 'row', gap: 4 }}>
                    {[0, 1].map(i => (
                      <TouchableOpacity key={i} activeOpacity={0.9} style={[s.slot, activeSlot === i && activeTool === 'select' && s.slotActive]} onPress={() => handleSlotPress(i)}>
                  ...
                  <View style={{ flex: 1, flexDirection: 'row', gap: 4 }}>
                    {[2, selectedLayout.id === '4_grid' ? 3 : null].filter(x => x !== null).map(i => (
                      <TouchableOpacity key={i} activeOpacity={0.9} style={[s.slot, activeSlot === i && activeTool === 'select' && s.slotActive]} onPress={() => handleSlotPress(i)}>
                        {images[i] ? <Image source={{ uri: images[i] }} style={s.slotImage} /> : <View style={s.emptySlot}><ImagePlus size={24} color={Colors.textSecondary} /><Text style={s.emptySlotText}>Tap to add</Text></View>}
                      </TouchableOpacity>
                    ))}
          ...
          {/* Touch Interceptor for Drawing/Text mode */}
          {activeTool !== 'select' && (
            <View style={[StyleSheet.absoluteFill, { zIndex: 10 }]} {...(activeTool === 'marker' ? canvasPan.panHandlers : textTapPan.panHandlers)} />
          )}
        </View>
...
with open('src/components/CollageMaker.js', 'w') as f:
    f.write(code)
