...
const CustomerOrdersScreen = ({ navigation }) => {
  ...
  const customerOrders = React.useMemo(() => {
    ...
    return sorted.filter(order => {
      ...
      if (selectedTab === 'stitching') {
        return type === 'TAILORING' || type === 'STITCHING' || type === 'CUSTOM' || type === 'ENQUIRY';
      } else {
        return (
  ...
  const getStatusColor = status => {
    ...
    if (text.includes('Pending') || text.includes('Yet To Start'))
      return { color: '#F97316', bg: '#FFF7ED', step: 1 };
    if (text.includes('Stitching') || text.includes('Progress'))
      return { color: '#3B82F6', bg: '#EFF6FF', step: 2 };
    if (text.includes('Quality') || text.includes('Check'))
      return { color: '#8B5CF6', bg: '#F5F3FF', step: 3 };
    if (text.includes('Ready') || text.includes('Completed'))
      return { color: '#10B981', bg: '#F0FDF4', step: 3 };
    if (text.includes('Delivered'))
      return { color: '#14B8A6', bg: '#F0FDFA', step: 4 };
  ...
  const introSlides = [
    {
      title: 'Your Orders',
      subtitle:
        'Keep a clean and smooth tab on the stitching and delivery status of all your designer blouses, lehengas, and outfits in real-time.',
    ...
    },
    {
      ...
      subtitle:
        'Instantly share design sketches, sleeve references, and neck patterns directly with the boutique tailors without WhatsApp clutter.',
    ...
    },
    {
      ...
      subtitle:
        'Courier your favorite sample garments to record and lock your measurements history for future stitching orders.',
  ...
  const renderOrderItem = ({ item }) => {
    ...
    const orderLabel =
      item.billNo ||
      item.order_number ||
      (isSale ? `INV-${item.id}` : `ORD-${item.id}`);

    const outfits = item.outfits || item.items || [];
    const hasPendingPhotoRequest =
      !isSale &&
      outfits.some(
        outfit =>
    ...
    return (
      ...
      >
        ...
        {/* Horizontal Divider */}
        <View
          style={{ height: 1, backgroundColor: '#E2E8F0', marginBottom: 12 }}
        />
        ...
        {hasPendingPhotoRequest && (
          ...
          >
            ...
            <Text style={[styles.photoRequestText, { fontSize: 12 }]}>
              PHOTO NEEDED
            </Text>
          </View>
        )}
      ...
      >
        <TouchableOpacity
          style={[
            ...
            selectedTab === 'stitching' && { backgroundColor: '#EEF2FF' },
          ]}
          onPress={() => setSelectedTab('stitching')}
        >
          <Text
            style={[
              { fontFamily: 'Inter-SemiBold', fontSize: 14, color: '#64748B' },
              selectedTab === 'stitching' && { color: '#5B43EE' },
            ]}
          >
            Stitching
          </Text>
        ...
        <TouchableOpacity
          style={[
            ...
            selectedTab === 'ready-made' && { backgroundColor: '#EEF2FF' },
          ]}
          onPress={() => setSelectedTab('ready-made')}
        >
          <Text
            style={[
              { fontFamily: 'Inter-SemiBold', fontSize: 14, color: '#64748B' },
              selectedTab === 'ready-made' && { color: '#5B43EE' },
            ]}
          >
            Ready-Made
          </Text>
          ...
          <View style={styles.modalCard}>
            ...
            <View
              style={[
                ...
                { backgroundColor: introSlides[introIndex].bg },
              ]}
            >
              ...
              <Text style={styles.slideTitle}>
                {introSlides[introIndex].title}
              </Text>
              <Text style={styles.slideSubtitle}>
                {introSlides[introIndex].subtitle}
              </Text>
            </View>
