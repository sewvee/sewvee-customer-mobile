...
function patchFile(filePath) {
  ...
  // Regex to match the mobile number input row
  const regex = /<Ionicons name="call-outline" size=\{20\} color="#94A3B8" style=\{styles\.inputIcon\} \/>\s*<TextInput style=\{styles\.textInput\} placeholder="10-digit number" placeholderTextColor="#94A3B8" keyboardType="number-pad" maxLength=\{10\} value=\{phone\} onChangeText=\{\(v\) => \{setPhone\(v\.replace\(\/\[\^0-9\]\/g, ''\)\); setErrorMsg\(''\);\}\} \/>/m;
