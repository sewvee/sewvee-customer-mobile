...
function patchFile(filePath, isSignup) {
  ...
  if (isSignup) {
    ...
    // In login, it probably uses `phone` to send to backend or loginContext
    content = content.replace(
      /customerLogin\(phone, /g,
      "customerLogin(countryCode === '+91' ? phone : countryCode + phone, "
    );
  }

  // Find Phone input UI
  const phoneUI = `<Icon name="phone" size={20} color="#94A3B8" style={styles.inputIcon} />
                        <TextInput
                            style={styles.input}
  ...
  if (content.includes('10-digit number')) {
      content = content.replace(phoneUI, newPhoneUI);
...
patchFile('src/screens/CustomerSignupScreen.js', true);
patchFile('src/screens/CustomerLoginScreen.js', false);
